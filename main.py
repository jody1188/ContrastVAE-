...
from models import ContrastVAE, ContrastVAE_VD, OfflineItemSimilarity, OnlineItemSimilarity, S3RecModel
...
def main():
    ...
    parser.add_argument('--data_name', default='Tools_and_Home_Improvement', type=str)
    parser.add_argument('--do_eval', action='store_true')
    parser.add_argument('--ckp', default=10, type=int, help="pretrain epochs 10, 20, 30...")


    # model args
    parser.add_argument("--model_name", default='ContrastVAE_VD', type=str)
    ...
    parser.add_argument('--diri', default=0, type=int)
    parser.add_argument('--context', default=0, type=int)
    parser.add_argument('--attn_block', default=2, type=int)
    ...
    parser.add_argument("--mim_prob", default=0.99, type=float)
    ...
    parser.add_argument('--Joint_VAE', action='store_true')
    ...
    # MiM Model
    args.mim_model = S3RecModel(args)
    pretrained_path = os.path.join(args.output_dir, f'{args.data_name}-epochs-{300}.pt')
    args.mim_model.load_state_dict(torch.load(pretrained_path))
    ...
    train_dataset = RecWithContrastiveLearningDataset(args, 
                                    user_seq[:int(len(user_seq)*args.training_data_ratio)], data_type='train')
