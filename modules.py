...
class SelfAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask,context = None):
        mixed_key_layer = self.key(input_tensor)
        if type(context) == torch.Tensor:
            mixed_value_layer = self.value(context)
            mixed_query_layer = self.query(context)
        else:
...
class Layer(nn.Module): # attention block
    ...
    def forward(self, hidden_states, attention_mask,context = None):
        if type(context) == torch.Tensor:
            attention_output = self.attention(hidden_states, attention_mask,context)
            intermediate_output = self.intermediate(attention_output)
            if self.args.attn_block >= 2:
                for i in range(self.args.attn_block-1):
                    attention_output = self.attention(intermediate_output, attention_mask,context)
                    intermediate_output = self.intermediate(attention_output)
        else:
...
class Decoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True,context = None):
        """
        ...
        if type(context) == torch.Tensor:
            all_decoder_layers = []
            for layer_module in self.layer:
                hidden_states = layer_module(hidden_states, attention_mask,context)
                if output_all_encoded_layers:
