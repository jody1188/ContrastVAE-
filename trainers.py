...
from utils import recall_at_k, ndcg_k, get_metric, cal_mrr, get_user_performance_perpopularity, get_item_performance_perpopularity, nCr
...
class ContrastVAETrainer(Trainer):
    ...
    def _one_pair_contrastive_learning(self, inputs):
        ...
        reconstructed_seq1, reconstructed_seq2, mu1, mu2, log_var1, log_var2, z1, z2 = self.model.forward(cl_batch, self.step)

        batch_size = cl_batch.shape[0]//2

        cl_output_slice = torch.split(torch.sum(z2, dim = 1), batch_size) 

        cl_loss = self.cl_criterion(cl_output_slice[0], 
                                cl_output_slice[1])
    ...
    def iteration(self, epoch, dataloader, full_sort=False, train=True):
        ...
        if train:
            ...
            for i, (rec_batch, cl_batches) in rec_cf_data_iter:
                ...
                if self.args.Joint_VAE:
                    reconstructed_seq1, reconstructed_seq2, mu1, mu2, log_var1, log_var2, z1, z2 = self.model.forward(input_ids, self.step)
                ...
                if full_sort:
                    ...
                    for i, batch in rec_data_iter:
                        ...
                        if self.variational_dropout:
                            recommend_reconstruct1, mu1, log_var1, z1, z2, alpha= self.model.forward(input_ids,0, self.step)
                            if self.args.store_latent:
                            ...
                            recommend_reconstruct1, recommend_reconstruct2, mu1, mu2, log_var1, log_var2, z1, z2 = self.model.forward(input_ids, self.step)
                            if self.args.store_latent:
...
class PretrainTrainer(Trainer):
    ...
    def pretrain(self, epoch, pretrain_dataloader):
        ...
        for i, batch in pretrain_data_iter:
            # 0. batch_data will be sent into the device(GPU or CPU)
            batch = tuple(t.to(self.device) for t in batch)
            masked_item_sequence, items = batch

            logits = self.model.pretrain(masked_item_sequence)
            logits = logits.view(-1, logits.size(-1))
            items = items.view(-1) 
            mim_loss = self.criterion(logits, items)

            self.optim.zero_grad()
            mim_loss.backward()
            self.optim.step()

            mim_loss_avg += mim_loss.item()

            num = len(pretrain_data_iter) * self.args.pre_batch_size
            post_fix = {
            "epoch": epoch,
            "mim_loss_avg": '{:.4f}'.format(mim_loss_avg /num),
            }
            print(desc)
            print(str(post_fix))
            with open(self.args.log_file, 'a') as f:
