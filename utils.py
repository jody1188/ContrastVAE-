...
from data_augmentation import _ensmeble_sim_models
...
def more_neg_sample(item_set, item_size, item_similarity_model):

    if type(item_similarity_model) is list:

        item_sim_model_1 = item_similarity_model[0]
        item_sim_model_2 = item_similarity_model[1]
        ensemble = True
    else:
        item_similarity_model = item_similarity_model
        ensemble = False

    for index in item_set:
        if ensemble:
            bottom_k_one = item_sim_model_1.most_similar(index, with_score = True)
            bottom_k_two = item_sim_model_2.most_similar(index, with_score = True)
            least_similar_items = _ensmeble_sim_models(bottom_k_one, bottm_k_two)
            item = least_similar_items[0]
            i = 0
            while item in item_set:
                if i == len(least_similar_items) - 1:
                    item = random.randint(1, item_size - 1)
                else:
                    item = least_similar_items[i+1]

        else:
            item = item_similarity_model.least_similar(index)[0]
            i = 0 
            while item in item_set:
                if i == len(item_similarity_model.least_similar(index)) - 1:
                    item = random.randint(1, item_size - 1)
                else:
                    item = item_similarity_model.least_similar(index)[i+1]
    return item
